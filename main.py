from pop3 import fetch_emails, extract_info, extract_mess, extract_file, get_subject_content, get_user_content, data
...
def save_file(path_file, path_save):
    ...
    #print_message("./database/2@gmail.com/Important/1072")
...
def send(ip, port, sender, pas):
    ...
    if attach_file == '1':
        ...
        # Thực hiện gửi email
        if to:
            send_email(ip, port, sender, pas, to, subject, content, files)
        if cc:
            cc_send_email(ip, port, sender, pas, cc, subject, content, files)
        if bcc:
            bcc_send_email(ip, port, sender, pas, bcc, subject, content, files)
        print("Đã gửi email thành công")
    else:
        if to:
            send_email(ip, port, sender, pas, to, subject, content)
        if cc:
            cc_send_email(ip, port, sender, pas, cc, subject, content)
        if bcc:
            bcc_send_email(ip, port, sender, pas, bcc, subject, content)
    ...
    if folder_choice.isdigit():
        ...
        if 1 <= folder_choice <= len(folder_list):
            ...
            if check_path(path_folder):
                ...
                print(f'Không có email nào trong {folder_name} này')
        else:
